	...
	<view>
		...
		<view class="cu-list menu" :class="['sm-border']">
			<view class="cu-item arrow"   :class="modalName=='move-box-'+ index?'move-cur':''"  
				 @tap="pu(pt.id)"   v-for="(pt,index) in labelList" :key="index"
				...
				<view class="move">
					<view class="bg-orange" @tap.stop="openUpdate(pt.id,pt.name)" >修改</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openUpdate(id,name){
				uni.navigateTo({
				    url: 'update/update?id='+id + "&name=" + name
				})
			},
			...
			openDelete(id){
				...
				this.vusui.confirm('您确定要删除当前记录吗？',{
					...
					that.vusui.load(3)
					that.http.post('/openapi/task/label/delete', {
						ids: id,
					}).then((res) => {
