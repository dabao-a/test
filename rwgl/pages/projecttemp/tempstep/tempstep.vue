	...
	<view>
		...
		<view class="cu-list menu" :class="['sm-border']">
			<view class="cu-item arrow"   :class="modalName=='move-box-'+ index?'move-cur':''"  
				 v-for="(pt,index) in tempstepList" :key="index"
				...
				<view class="action">
					<text class="icon iconfont" >第{{index+1}}步</text>
				</view>
				<view class="move">
...
<script>
	...
	export default {
		...
		methods: {
			...
			openAdd() {
				uni.navigateTo({ 
				    url: 'add/add?tempId=' + this.tempId
				})
			},
			
			// 列表
			listTempstep () {
				this.http.post(`/openapi/task/projecttemp/step/list/${this.tempId}`, {
					
				}).then((res) => {
					if(res.code != 0) return false
					this.tempstepList = res.rows;
				}) 
			},
			...
			openDelete(id){
				...
				this.vusui.confirm('您确定要删除当前记录吗？',{
					...
					that.vusui.load(3)
					that.http.post('/openapi/task/projecttemp/step/delete', {
						ids: id
					}).then((res) => {
