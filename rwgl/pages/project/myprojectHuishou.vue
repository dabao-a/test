	...
	<view>
		...
		<view class="cu-list menu" :class="['sm-border']">
			<view class="cu-item arrow"   :class="modalName=='move-box-'+ index?'move-cur':''"  
				...
				<view class="move">
					<view class="bg-orange" @tap.stop="openUpdate(pt.id)" >还原</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 列表
			listMyproject () {
				this.http.post('/openapi/task/project/listHuishou', {
			...
			openUpdate(id){
				var that = this
				this.vusui.confirm('您确定要还原当前记录吗？',{
					icon: 2
				}, function() {
					that.vusui.load(3)
			...
			openDelete(id){
				...
				this.vusui.confirm('您确定要删除当前记录吗？',{
					...
					that.vusui.load(3)
					that.http.post('/openapi/task/project/delete', {
						ids: id
					}).then((res) => {
						that.vusui.close("loading")
						if(res.code != 0) {
