<template>
	<view>
		<!-- 弹出层组件 -->
		<vus-layer></vus-layer>
		
		<view class="cu-list menu" :class="['sm-border']">
			<view class="cu-item arrow"   :class="modalName=='move-box-'+ index?'move-cur':''"  
				 v-for="(pt,index) in puList" :key="index"
				 @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + index">
				<view class="content">
					<view >{{pt.user.userName}}</view>
				</view>
				<view class="action">
					<text class="icon iconfont" style="font-size:50upx"
					:class="[pt.status=='0'?'icon-daishenhe':pt.status=='1'?'icon-tongguo':'icon-bohui',
							 pt.status=='0'?'text-yellow':pt.status=='1'?'base_fontcolor':'text-red']" ></text>
				</view>
				<view class="move">
					<view class="bg-red"    @tap.stop="openDelete(pt.id)" >删除</view>
				</view>
			</view>
		</view>
		<view class="uni-fab-box">
			<uni-fab ref="fab" :pattern="pattern"  horizontal="left" vertical="bottom" direction="horizontal" @fabClick="openAdd()" />
		</view>
	</view>
</template>

<script>
	import uniFab from '@/components/uni-fab-add.vue'
	export default {
		components: {
			uniFab
		},
		data() {
			return {
				modalName: null,
				listTouchStart: 0,
				listTouchDirection: null,
				title: '新增',
				pattern: {
					color: '#7A7E83',
					backgroundColor: '#fff',
					selectedColor: '#007AFF',
					buttonColor: '#007AFF'
				},
				projectId: '',
				puList: []
			}
		},
		onLoad(options) {
			this.projectId = options.projectId
		},
		created () {
			this.listPu()
		},
		onShow() {
			var pages = getCurrentPages();
			//当前页面的数据
			var currPage = pages[pages.length - 1];  
			var data = currPage.$vm.data
			if(data != null){
				var needReflush = data.needReflush;
				if(needReflush == 1){
					this.listPu()
					var datax = {
						needReflush : 0
					}
					currPage.$vm.data=datax
				}
			} 
		},
		methods: {
			// 进入新增界面
			openAdd() {
				uni.navigateTo({ 
				    url: 'add/add?projectId=' + this.projectId
				})
			},
			
			// 列表
			listPu () {
				this.http.post(`/openapi/task/project/user/list/${this.projectId}`, {
					
				}).then((res) => {
					if(res.code != 0) return false
					this.puList = res.rows;
				}) 
			},
			
			// 删除
			openDelete(id){
				var that = this
				this.vusui.confirm('您确定要删除当前记录吗？',{
					icon: 2
				}, function() {
					that.vusui.load(3)
					that.http.post('/openapi/task/project/user/delete', {
						ids: id
					}).then((res) => {
						that.vusui.close("loading")
						if(res.code != 0) {
							that.vusui.alert(res.msg)
							return false
						} else {
							that.vusui.alert('删除成功')
							that.listPu();
						}
					}) 
				})
			},
			
			// ListTouch触摸开始
			ListTouchStart(e) {
				this.listTouchStart = e.touches[0].pageX
			},
			// ListTouch计算方向
			ListTouchMove(e) {
				this.listTouchDirection = e.touches[0].pageX - this.listTouchStart > 0 ? 'right' : 'left'
			},
			// ListTouch计算滚动
			ListTouchEnd(e) {
				if (this.listTouchDirection == 'left') {
					this.modalName = e.currentTarget.dataset.target
				} else {
					this.modalName = null
				}
				this.listTouchDirection = null
			}
		}
	}
</script>
